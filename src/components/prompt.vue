  ...
  <div
    ...
    <div class="chat-container h-5/6 overflow-y-scroll p-4" ref="chatContainer">
      <transition-group name="chat" tag="div" class="chat-container gap-y-6 flex flex-col">
        <div v-for="(chat, i) in chats" :key="i" class="flex gap-x-3 items-start"
          :class="chat.direction == 'out' ? 'justify-end' : 'justify-start'">
          <div v-if="chat.direction == 'in'"
            class="h-fit w-fit text-base font-semibold rounded-full py-1 px-[0.4rem] bg-purple-200">
            🗿
          </div>
          <p class="mt-2 max-w-xs bg-gray-700 p-3 rounded-md"
            :class="chat.direction == 'in' ? 'text-left' : 'text-right'">
            {{ chat.message }}
          </p>
          <div v-if="chat.direction == 'out'"
            class="h-fit w-fit text-base font-semibold rounded-full py-1 px-[0.4rem] bg-white">
            🚀
          </div>
        </div>
    ...
    <div class="mt-8 w-full flex flex-row gap-x-4">
      ...
      <button @click="sendMessage()" :disabled="getLoading" class="md:px-5 px-2 py-3 rounded-md w-fit" :class="getLoading ? 'bg-gray-300' : 'bg-purple-300'">Send</button>
    </div>
  </div>
</template>
